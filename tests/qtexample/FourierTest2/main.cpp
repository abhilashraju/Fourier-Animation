...
//       syms.emplace_back(200._A * 1._F + 20._A * -2._F + 75._A * 4._F + 25._A * -5._F + 100._A * -3._F);
        ...
        std::vector<FReal> yvals;
         ...
         ftofx= IFT(xvals);
//         ftofx.clear();
//         ftofx.emplace_back(FTerm(50));
//         ftofx.emplace_back(FTerm(30,-2));
//         ftofx.emplace_back(FTerm(20,3));
         ftofy= IFT(yvals);
...
//        })));
        ...
        targetWave.emplace_front(FT(ftofx,{100,100},time,[&](auto c,auto per,auto amp)->void{
                        qDebug()<< offset+QPointF{c.x(),c.y()};
                        qDebug()<<offset+QPointF{per.x(),per.y()};
                        p.drawEllipse(offset+QPointF{c.x(),c.y()},amp,amp);
                        p.drawLine(offset+QPointF{c.x(),c.y()},offset+QPointF{per.x(),per.y()});
        }));
